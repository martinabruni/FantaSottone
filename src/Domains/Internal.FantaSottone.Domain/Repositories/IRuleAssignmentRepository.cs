...
public interface IRuleAssignmentRepository : IRepository<RuleAssignment, int>
{
    ...
    /// </summary>
    Task<AppResult<bool>> IsRuleAssignedAsync(int ruleId, CancellationToken cancellationToken = default);

    /// <summary>
    /// Gets assignment by rule ID
    /// </summary>
    Task<AppResult<RuleAssignment>> GetByRuleIdAsync(int ruleId, CancellationToken cancellationToken = default);
